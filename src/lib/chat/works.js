 ...
 *
 * Biased toward texts an Israeli group will actually feel something about —
 * the Hebrew and Jewish works are the load-bearing ones; the novels are flavour.
 * Every entry carries both a Hebrew and an English title.
 *
 ...
 * works therefore scale a Hebrew total by HEBREW_DENSITY first; comparisons
 * against `he` works use it as-is. An English-language chat is the mirror
 * image: `he` works are scaled down by the same factor.
 *
 * `confidence`:
...
/** @typedef {{ he: string, en: string, words: number, lang: "he" | "en", confidence: "high" | "medium" | "low" }} Work */
...
/**
 * @typedef {object} WordComparison
 * @property {number} totalWords
 * @property {number} hebrewDensityFactor
 * @property {number} torahMultiple
 * @property {{ he: string, en: string, words: number, ratio: number }} nearest
 * @property {{ he: string, en: string, words: number, ratio: number }[]} beats
 * @property {{ he: string, en: string, words: number, ratio: number }[]} shortOf
 */

/**
 * Compare a word total against the table.
 *
 ...
 * @param {"he" | "en"} [chatLang]  language the chat was written in; drives the density factor
 * @returns {WordComparison}
 */
export function compareToWorks(totalWords, chatLang = "he") {
  // Same language as the work: compare as-is. Hebrew chat vs English work:
  // scale up. English chat vs Hebrew work: scale down.
  const scored = WORKS.map((w) => {
    const effective =
      w.lang === chatLang ? totalWords : chatLang === "he" ? totalWords * HEBREW_DENSITY : totalWords / HEBREW_DENSITY;
  ...
  const pick = (w) => ({ he: w.he, en: w.en, words: w.words, ratio: Number(w.ratio.toFixed(2)) });
  return {
    totalWords,
    ...
    beats: beaten.slice(0, 6).map(pick),
    shortOf: ahead.slice(0, 4).map(pick),
  };
}
